...
public partial class ScanPage : ContentPage, IQueryAttributable
{
    ...
    private async Task DecodeLineItems()
    {
        ...
        else if (scannedBill?.OrderLines.Count == 0) // The normal case, probably as a result of taking a picture or scanning a new image
        {
            ...
            if (ImagePath is null)
                throw new InvalidOperationException("No image path provided for scanning");
            if (tokenSource is null)
                throw new InvalidOperationException("No cancellation token source available for scanning");
        ...
        // We should have a bill in scannedBill, either loaded from a scan we just did or (for debugging locally)
    ...
    private async void UpdateCurrentMeal(bool clearItems = false)
    {
        ...
        if (scannedBill?.FormElements is not null)
        {
            foreach (FormElement item in scannedBill.FormElements)
            {
                if (ScannedSubTotal == 0 && string.Equals(item.FieldName, "Subtotal", StringComparison.OrdinalIgnoreCase) && decimal.TryParse(item.FieldValue, out decimal d))
                {
                    ScannedSubTotal = d;
                    if (ScannedTax > 0)
                        break;
                }
                else if (ScannedTax == 0 && string.Equals(item.FieldName, "TotalTax", StringComparison.OrdinalIgnoreCase) && decimal.TryParse(item.FieldValue, out decimal t))
                {
                    ScannedTax = t;
