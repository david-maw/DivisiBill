...
> keytool -list -v -keystore new.keystore -alias -storepass xxxx
...
After a successful automated build, the Google service account is retrieved from an environment variable and used to deploy the app to the play store "alpha" stream. This starts with putting the service account into an environment variable. The one used is
